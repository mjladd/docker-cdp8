...
1. Binaries output to `docs/legacy/NewRelease/` and are automatically on `PATH` in the devcontainer, so you can run tools from anywhere:
...
* The devcontainer adds `/workspaces/<repo>/docs/legacy/NewRelease` to `PATH` by default.
...
For native builds on macOS, Linux, or Windows, see [docs/legacy/building.txt](docs/legacy/building.txt) for full details.
...
```bash
# 1. Build and install PortAudio first (see docs/legacy/dev/externals/pa*build.txt for details)

# 2. Create build directory
mkdir build && cd build

# 3. Generate makefiles
cmake ../docs/legacy                          # macOS/Linux/MSVC
cmake -G "MinGW Makefiles" ../docs/legacy     # Windows MinGW
...
Compiled binaries are written to the `docs/legacy/NewRelease/` directory.
...
# Native (from docs/legacy/NewRelease directory):
